...
function findIndexByProperty(arr, name) {
  ...
  let foundIndex = arr.findIndex((obj) => obj.name === name);
...
function getArrayKeys(arr) {
  ...
  let arrayKeys = Array.from(iterator);
  return arrayKeys;
...
function getPositiveArrayValues(arr) {
  ...
  let arrayValues = Array.from(iterator);
  let positiveArrayValues = arrayValues.filter((arr) => arr > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let array = arr.flat();
  let flattenSquare = array.map((array) => Math.pow(array, 2));
  let filteredSquaredArray = flattenSquare.filter(
    (flattenSquare) => flattenSquare > threshold
  );
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let getLastElements = arr.slice(-n);
  let sum = getLastElements.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue
  );
  getLastElements.forEach((getLastElements) => console.log(getLastElements));
  return sum;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function wordLengths(arr) {
  ...
  let lengthsArray = arr.map((arr) => arr.length);
  return lengthsArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
...
function findLongestWord(arr) {
  ...
  let longestWord = arr.reduce((longest, word) =>
...
function findDuplicateElements(arr) {
  ...
  let duplicateElements = arr.filter((num, index) => {
...
function calculateTotalPrice(arr) {
  ...
  let totalPrice = arr.reduce((total, product) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  let lastIndex = arr.findLastIndex((element) => element < 0);
  return lastIndex;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
